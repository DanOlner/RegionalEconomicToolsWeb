---
title: "GDP regional gaps in the UK"
execute:
  echo: true
  warning: false
  error: false
...
#Of more interest might be the change in rank position between the different measures.
#Rank position per measure type
pip <- pip %>% 
  group_by(measure) %>% 
  mutate(
    rank = rank(value)
...
#How many geog regions here? 46
length(unique(pip$Place))

#Can have a look at spread of rank positions for all places?
#Order by rank range (biggest range = place changes rank position the most)
ggplot(pip, aes(x = fct_reorder(Place, rankrange), y = rank)) +
  geom_boxplot() +
  coord_flip()
...
#Different measures also reduce the RELATIVE distances between places 
#E.g. London is less of an outlier (though still top rank) for various measures
#Relative distances probably better, as I don't think the nominal index values are likely easily comparable
#(but need to check that)

#Repeat of plot above, with some of the largest rank spreads shown
top_places <- pip %>%
  ungroup() %>% 
...
#Interesting - for Oxford and Tees Valley, opposite positions and they swap over
ggplot() +
  geom_jitter(data = pip, aes(x = measure, y = rank, group = Place), width = 0.1, size = 3) +
  geom_jitter(data = pip %>% filter(Place %in% top_places[3:4]), aes(x = measure, y = rank, group = Place, colour = Place), width = 0.1, size = 5, shape =15) +
  # geom_hline(yintercept = 100, colour = 'red', size = 3, alpha = 0.25) +
  coord_flip() +
  xlab('') + ylab('rank position')


#Seeing what seems to be a geographical north-south pattern for which measure types and their rank range dominate
#i.e. Oxfordshire and Gloucestershire start out with higher rank than LCR and Tees, but those swap over entirely

#Look at that directly for top rank movers?
p <- ggplot(pip %>% filter(Place %in% top_places) %>% ungroup() %>% mutate(Place = fct_reorder(Place,-rankrange,mean)), aes(x = measure, y = rank, group = Place, colour = Place)) +
  geom_line(size = 2) +
  geom_point(size = 3) +
...
#Mapping is a bit to piece together:
#"For England, the sub-regions we use are the 38 Local Enterprise Partnership areas. 
#For Scotland and Wales, we use NUTS2 regions (5 in Scotland, 2 in Wales) and Northern Ireland is treated as a single unit."

#REMINDER, THIS IS NOT BEING EXECUTED!
